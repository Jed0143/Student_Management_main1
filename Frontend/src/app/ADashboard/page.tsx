...
const Adashboard = () => {
  ...
  const handleSetSchedule = (id: string | number) => {
    const studentData = acceptedStudents.find((stu) => Number(stu.id) === Number(id));
    if (studentData) {
  ...
  const handleSaveSchedule = () => {
    if (selectedStudent && selectedSchedule) {
      ...
      fetch("http://localhost/Student_Management_main1/backend/save_student_schedule.php", {
        method: "POST",
        headers: {
      ...
      alert("Please select a student and schedule.");
    }
  };
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 p-6 bg-blue-200 min-h-screen">
        ...
        <section className="bg-white p-6 rounded-lg shadow-md max-w-6xl mx-auto">
          ...
          <p className="mb-4 text-sm text-gray-600">
            Available shifts: <strong>7AM-9PM</strong>, <strong>9AM-11PM</strong>,{" "}
            <strong>1AM-3PM</strong>
          </p>
      ...
      <Dialog open={openScheduleDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
            ...
            {["7AM-9PM", "9AM-11PM", "1AM-3PM"].map((slot) => (
              <button
                ...
                className={`w-full py-2 px-4 rounded border ${
                  selectedSchedule === slot ? "bg-blue-500 text-white" : "bg-gray-200"
                }`}
              >
                {slot}
              </button>
        ...
        <DialogActions>
          ...
          <Button onClick={handleSaveSchedule} color="primary" disabled={!selectedSchedule}>
            Save
          </Button>
        </DialogActions>
      </Dialog>
